...
class SubSelectorComponent(ModeSelectorComponent):
	...
	def update(self):
		...
		if self.is_enabled():
			...
			if len(self.sendOffsetCurrentPattern) > 7:
				self.sendOffsetCurrentPattern = []
			...
			if self._mode_index == -1:
				...
				self.sendOffsetCurrentPattern.append(0)
				
			elif self._mode_index == 1:
				self._setup_pan_mode()
				self.sendOffsetCurrentPattern.append(1)				
			elif self._mode_index == 2:
				self.sendOffsetCurrentPattern.append(2)
				if len(self.sendOffsetCurrentPattern) > 0 and self.sendOffsetCurrentPattern == self.sendOffsetPattern:
					self.sendOffset -= 1
					self.sendOffsetCurrentPattern = []
				if self.sendOffset < 0:
					self.sendOffset = 0
				if self.sendOffset == 0:
					self._setup_send1_mode()
				elif self.sendOffset == 1:
					self._setup_send2_mode()
				elif self.sendOffset == 2:
					self._setup_send3_mode()
				elif self.sendOffset == 3:
					self._setup_send4_mode()
				elif self.sendOffset == 4:
					self._setup_send5_mode()
			elif self._mode_index == 3:
				self.sendOffsetCurrentPattern.append(3)#
				if len(self.sendOffsetCurrentPattern) > 0 and self.sendOffsetCurrentPattern == self.sendOffsetBPattern:
					self.sendOffset += 1
					self.sendOffsetCurrentPattern = []
				if self.sendOffset > 4:
					self.sendOffset = 4
				if self.sendOffset == 0:
					self._setup_send2_mode()
				elif self.sendOffset == 1:
	...
	def _setup_send3_mode(self):
		for track in range(self._matrix.width()):
			...
			self._sliders[track].set_mode(SLIDER_MODE_VOLUME)
			self._sliders[track].set_value_map(SEND_VALUE_MAP)
			strip.set_send_controls((None, None, self._sliders[track]))

		self._session.set_stop_track_clip_buttons(None)
		self._session.set_stop_all_clips_button(None)
		self._mixer.set_global_buttons(None, None, None)
	def _setup_send4_mode(self):
		for track in range(self._matrix.width()):
			strip = self._mixer.channel_strip(track)
			strip.set_default_buttons(None, None, None, None)
			strip.set_mute_button(None)
			...
			self._sliders[track].set_mode(SLIDER_MODE_VOLUME)
			self._sliders[track].set_value_map(SEND_VALUE_MAP)
			strip.set_send_controls((None, None, None, self._sliders[track]))

		self._session.set_stop_track_clip_buttons(None)
		self._session.set_stop_all_clips_button(None)
		self._mixer.set_global_buttons(None, None, None)
	def _setup_send5_mode(self):
		for track in range(self._matrix.width()):
			strip = self._mixer.channel_strip(track)
			strip.set_default_buttons(None, None, None, None)
			strip.set_mute_button(None)
			...
			self._sliders[track].set_mode(SLIDER_MODE_VOLUME)
			self._sliders[track].set_value_map(SEND_VALUE_MAP)
			strip.set_send_controls((None, None, None, None, self._sliders[track]))

		self._session.set_stop_track_clip_buttons(None)
		self._session.set_stop_all_clips_button(None)
		self._mixer.set_global_buttons(None, None, None)
	def _setup_send6_mode(self):
		for track in range(self._matrix.width()):
			strip = self._mixer.channel_strip(track)
			strip.set_default_buttons(None, None, None, None)
			strip.set_mute_button(None)
			...
			self._sliders[track].set_mode(SLIDER_MODE_VOLUME)
			self._sliders[track].set_value_map(SEND_VALUE_MAP)
			strip.set_send_controls((None, None, None, None, None, self._sliders[track]))
